...
do
  case $menu  in
        "Session's PostgreSQL")
          toggleContainer "postgresql" "POSTGRESQL_PID" "-p 5432:5432 -e POSTGRESQL_PASSWORD=confi_user -e POSTGRESQL_USER=conference_user -e POSTGRESQL_ADMIN_PASSWORD=conference -e POSTGRESQL_DATABASE=conference centos/postgresql-10-centos7:latest"
            ;;
        "Session")
        ...
        "Speaker")
          toggleService "speaker" "SPEAKER_PID"
            ;;
        "Votes's MongoDB")
          toggleContainer "mongo" "MONGO_PID" "-p 27017:27017 mongo:4.0"
            ;;            
        "Vote")
